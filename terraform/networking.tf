...
resource "aws_security_group" "app_server" {
  ...
  description = "Security group for Chatify application server"
  vpc_id      = aws_vpc.main.id
...
# ── Security Group: MongoDB (internal only) ────────────────────────────────

resource "aws_security_group" "mongodb" {
  name        = "${var.project_name}-${var.environment}-mongo-sg"
  description = "Security group for MongoDB — internal access only"
  vpc_id      = aws_vpc.main.id
